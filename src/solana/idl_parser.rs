...
use std::error::Error;
use crate::solana::structs::{Idl, IdlAccount, IdlTypeDefinition, IdlInstruction, IdlType, IdlField};
use sha2::{Sha256, Digest};
use solana_idl::IdlInstruction as SolIDLInstruction;
...
pub fn decode_idl_data (idl_json: &str) -> Result<(), Box<dyn Error>> {
    ...
    // TODO -- Add any checks required for panics (like empty case etc, request fuzzing checks, check json parsers to see what they do)

    let instructions = validate_idl_array(&idl_map, IDL_INSTRUCTIONS_KEY)?;
    for i in instructions {
        parse_single_instruction(i)?;
    }
...
fn check_for_idl_bools(idl_map: &Map<String, Value>, keys: &[&str]) -> bool {
    keys.iter()
...
fn compute_discriminator(instruction_name: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    let seed = format!("global:{}", instruction_name);
    let mut hasher = Sha256::new();
    hasher.update(seed.as_bytes());
    let result = hasher.finalize();
    result[..IDL_INST_DEFAULT_DISC_LEN].try_into().map_err(|_| Box::<dyn std::error::Error>::from(format!("Failed to compute instruction byte discriminator for instruction: {}", instruction_name)))
}
...
fn parse_single_account(account: Value) -> Result<IdlAccount, Box<dyn Error>> {
    ...
    let is_mut = check_for_idl_bools(acct_map, IDL_ACCOUNT_MUTABLE_KEYS);
    let is_signer = check_for_idl_bools(acct_map, IDL_ACCOUNT_SIGNER_KEYS);
    let is_optional = check_for_idl_bools(acct_map, IDL_ACCOUNT_OPTIONAL_KEYS);
    Ok(IdlAccount{
        name,
        is_mut,
        is_signer,
