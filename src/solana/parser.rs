use super::structs::{
    AccountAddress, CustomIdlConfig, IdlParseError, IdlRecord, SolTransfer, SolanaAccount,
    SolanaAddressTableLookup, SolanaInstruction, SolanaMetadata, SolanaParseResponse,
...
/// Entrypoint to parsing (legacy API)
///
/// # Arguments
...
/// Parse a Solana transaction with custom IDLs
///
/// This is the preferred API for passing custom IDLs. It supports both pre-parsed
/// `Idl` structs and JSON strings via `CustomIdlConfig`.
///
/// # Arguments
/// * `unsigned_tx` - Hex string of the transaction to parse
/// * `full_transaction` - Whether the input is a full transaction or just a message
/// * `custom_idls` - Optional map of program_id -> CustomIdlConfig
///   - Use `CustomIdlConfig::from_idl(idl, override)` to pass a pre-parsed Idl struct
///   - Use `CustomIdlConfig::from_json(json, override)` to pass a JSON string
///   - Pass `None` to use only built-in IDLs (default behavior)
///
/// # Example
/// ```ignore
/// use solana_parser::{parse_transaction_with_idls, CustomIdlConfig, Idl};
/// use std::collections::HashMap;
///
/// // Using a pre-parsed IDL
/// let idl: Idl = /* ... */;
/// let mut custom_idls = HashMap::new();
/// custom_idls.insert(
///     "program_id".to_string(),
///     CustomIdlConfig::from_idl(idl, true), // override built-in
/// );
///
/// let result = parse_transaction_with_idls(tx_hex, true, Some(custom_idls));
/// ```
pub fn parse_transaction_with_idls(
    unsigned_tx: String,
    full_transaction: bool,
    ...
    tx.message.sanitize().map_err(|e| {
        Box::<dyn std::error::Error>::from(format!(
            "Solana transaction message failed sanitization check: {e}"
        ))
    })?;
...
*/
#[allow(dead_code)] // Kept for backwards compatibility, use parse_solana_transaction_with_idls
...
fn parse_solana_transaction_with_idls(
    unsigned_tx: &str,
    full_transaction: bool,
    custom_idls: Option<HashMap<String, CustomIdlConfig>>,
) -> Result<SolanaTransaction, Box<dyn std::error::Error>> {
...
// Checks whether a compact-encoded array length exceeds the byte length of the
// remaining transaction. Every instruction consumes at least one byte, so this
// also prevents an inflated count from causing an excessive allocation.
fn verify_compact16_max_length<'a>(
    length: usize,
    bytes: &'a [u8],
    section: &str,
) -> Result<(usize, &'a [u8]), Box<dyn std::error::Error>> {
    let bytes_len = bytes.len();
    if bytes_len < length {
        return Err(format!(
            "error parsing unsigned transaction: indicated compact-encoded array length {length} exceeds transaction byte length {bytes_len} for {section}"
        )
        .into());
    }
    Ok((length, bytes))
...
fn parse_instructions(
    ...
    let (insts_num, tx_body_remainder) =
        read_compact_u16(tx_body_remainder).and_then(|(length, remaining_bytes)| {
            verify_compact16_max_length(length, remaining_bytes, "Instructions")
        })?;
    let mut compiled_insts: Vec<CompiledInstruction> = Vec::with_capacity(insts_num);
    let mut parsed_tx_body_remainder = tx_body_remainder;
...
fn read_compact_u16(
    ...
    loop {
        ...
        if bytes_read >= tx_body_remainder.len() {
            return Err(
                "error parsing unsigned transaction: unable to parse compact array header, not enough bytes".into()
            );
        }
        ...
        if shift == 14 && (byte & 0xfc) != 0 {
            return Err(
                "error parsing unsigned transaction: third byte in compact u16 has more than it's 2 least significant bits set".into()
            );
        }
        ...
        Ok(match tag {
            3 => {
                let (amount, _rest) = unpack_u64(rest)
                    .map_err(|_| "error while parsing spl instruction Transfer -- amount")?;
                Self::Transfer { amount }
            }
            12 => {
                let (amount, rest) = unpack_u64(rest)
                    .map_err(|_| "error while parsing spl instruction TransferChecked -- amount")?;
...
impl SolanaTransaction {
    #[allow(dead_code)] // Kept for backwards compatibility, use new_with_idls
    pub fn new(
        hex_tx: &str,
        full_transaction: bool,
        custom_idls: Option<HashMap<String, (String, bool)>>,
    ...
    */
        ...
        for i in self.message.instructions() {
            ...
            for a in i.accounts.clone() {
                ...
                if a as usize >= self.message.static_account_keys().len() {
                    ...
                    all_transaction_addresses
                        .push(AccountAddress::AddressTableLookUp(atlu.clone()));
                    continue;
                }
            ...
            if i.program_id_index as usize >= self.message.static_account_keys().len() {
                return Err(Box::<dyn std::error::Error>::from(
                    "Solana Instruction program index must be within static account keys",
                ));
            }
            ...
            match program_key.as_str() {
                SOL_SYSTEM_PROGRAM_KEY => {
                    ...
                    if let SystemInstruction::Transfer { lamports } = system_instruction {
                        // NOTE: While Solana system program Transfer instructions require 2 addresses (from and to), the Solana runtime is permissive and allows for the instructions to include more than 2 addresses
                        // In the case that more than 2 addresses are included, they are ignored by the Solana runtime, so the same is done here
                ...
                TOKEN_PROGRAM_KEY => {
                    ...
                    if let Some(spl_transfer) = self.parse_spl_instruction_data(
                        token_program_instruction,
                        all_transaction_addresses.clone(),
                    )? {
            ...
            let (parsed_inst_option, idl_parse_error) = parse_idl(
                &program_key,
                &all_transaction_addresses,
                i,
                &self.custom_idl_records,
            )?;
    ...
    fn parse_spl_instruction_data(
        ...
        all_transaction_addresses: Vec<AccountAddress>,
    ...
    fn get_spl_multisig_signers_if_exist(
        ...
        if all_transaction_addresses.len() < num_accts_before_signer {
            return Err(
                "invalid number of accounts provided for spl token transfer instruction".into(),
            );
        }
        Ok(all_transaction_addresses[num_accts_before_signer..]
            .iter()
            .map(|a| a.to_string())
            .collect())
    }
...
/// Parses instruction data using the IDL for the given program.
/// IDLs are embedded at compile time - no file system access required.
///
/// Returns `Ok((parsed, None))` on success, or `Ok((None, Some(error)))` when
/// IDL parsing fails in a structured way the caller can inspect.
fn parse_idl(
    program_key: &str,
    all_transaction_addresses: &[AccountAddress],
    inst: &CompiledInstruction,
    custom_idls: &HashMap<String, IdlRecord>,
) -> Result<(Option<SolanaParsedInstructionData>, Option<IdlParseError>), Box<dyn std::error::Error>>
{
    if let Some(idl_record) = custom_idls.get(program_key) {
        // Resolve which IDL to use (built-in or custom) - uses embedded IDLs
        let (idl, idl_json_str, idl_source) =
            match idl_parser::resolve_idl_for_record(idl_record, program_key) {
                Ok(v) => v,
                Err(e) => {
                    return Ok((None, Some(IdlParseError::IdlResolutionError(e.to_string()))));
                }
            };
        ...
        let instruction = match idl_parser::find_instruction_by_discriminator(
            &inst.data,
            ...
            Err(e) => {
                return Ok((
                    None,
                    Some(IdlParseError::DiscriminatorNotFound(e.to_string())),
                ));
            }
        };

        // Parse data into args map
        let parsed_args = match idl_parser::parse_data_into_args(&inst.data, &instruction, &idl) {
            Ok(v) => v,
            Err(e) => {
                return Ok((
                    None,
                    Some(IdlParseError::DataParseError {
        ...
        let named_accounts =
            match idl_parser::create_accounts_map(all_transaction_addresses, &instruction) {
                Ok(v) => v,
                Err(e) => {
                    return Ok((
                        None,
                        Some(IdlParseError::AccountsMapError {
                            instruction_name: instruction.name,
                            error: e.to_string(),
                        }),
                    ));
        ...
        if let Some(discriminator_bytes) = instruction.discriminator {
            return Ok((
                Some(SolanaParsedInstructionData {
        ...
        return Err("attempted to parse IDL into instruction without discriminator".into());
    }
    Ok((None, None))
}
...
mod tests {
    ...
    #[test]
    fn parses_transaction_with_128_instructions() {
        let signer = Pubkey::new_unique();
        let recipient = Pubkey::new_unique();
        let system_program = Pubkey::new_from_array([0; 32]);
        ...
        let mut instructions = vec![CompiledInstruction {
            program_id_index: 2,
            accounts: vec![0, 1],
            data: transfer_data,
        }];
        ...
        for message in [
            VersionedMessage::Legacy(LegacyMessage {
                ...
                account_keys: vec![signer, recipient, system_program, noop_program],
                recent_blockhash: Hash::default(),
                instructions: instructions.clone(),
            }),
            VersionedMessage::V0(VersionZeroMessage {
                header: MessageHeader {
                    num_required_signatures: 1,
                    num_readonly_signed_accounts: 0,
                    num_readonly_unsigned_accounts: 2,
            ...
            let mut tx_bytes = vec![1u8];
            tx_bytes.extend_from_slice(&[0u8; LEN_SOL_SIGNATURE_BYTES]);
            tx_bytes.extend_from_slice(&message.serialize());

            let parsed_tx = SolanaTransaction::new(&hex::encode(tx_bytes), true, None).unwrap();
            let metadata = parsed_tx.transaction_metadata().unwrap();

            assert_eq!(metadata.instructions.len(), 128);
            assert_eq!(metadata.transfers.len(), 1);
            assert_eq!(metadata.transfers[0].from, signer.to_string());
            assert_eq!(metadata.transfers[0].to, recipient.to_string());
            assert_eq!(metadata.transfers[0].amount, "100");
        }
    }

    #[test]
    fn rejects_transaction_with_inflated_instruction_count() {
        let signer = Pubkey::new_unique();
        let recipient = Pubkey::new_unique();
        let system_program = Pubkey::new_from_array([0; 32]);
        ...
        let message = VersionedMessage::Legacy(LegacyMessage {
            ...
            account_keys: vec![signer, recipient, system_program],
            recent_blockhash: Hash::default(),
        ...
        let mut message_bytes = message.serialize();
        let instruction_count_offset = LEN_MESSAGE_HEADER_BYTES
            + LEN_ARRAY_HEADER_BYTES
            + (3 * LEN_SOL_ACCOUNT_KEY_BYTES)
            + LEN_SOL_ACCOUNT_KEY_BYTES;
        message_bytes[instruction_count_offset] = 0x7f;

        let mut tx_bytes = vec![1u8];
        tx_bytes.extend_from_slice(&[0u8; LEN_SOL_SIGNATURE_BYTES]);
        tx_bytes.extend_from_slice(&message_bytes);

        let error = SolanaTransaction::new(&hex::encode(tx_bytes), true, None).unwrap_err();
        assert_eq!(
            error.to_string(),
            "Error parsing full transaction. If this is just a message instead of a full transaction, parse using the --message flag. Parsing Error: \"error parsing unsigned transaction: indicated compact-encoded array length 127 exceeds transaction byte length 17 for Instructions\""
        );
    }
    ...
    fn solana_fuzzer_regression_test3() {
        ...
        let unsigned_tx_instruction_problem1_raw = vec![
            0x01, 0x01, 0x03, 0x00, 0x0a, 0x0a, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00, 0x66, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8e, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x82, 0x82, 0x82, 0x82, 0x82, 0x82, 0x82, 0x00, 0x00, 0xf1, 0xf1,
            0xf1, 0xf1, 0xf1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x00, 0x00,
            0x66, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x8e, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x82, 0x82, 0x82, 0x82, 0x82, 0x82, 0x82, 0x00, 0x00,
            0xf1, 0xf1, 0xf1, 0xf1, 0xf1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0xef, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ];
        let unsigned_tx_instruction_problem1_hex_string =
            hex::encode(&unsigned_tx_instruction_problem1_raw);
        let unsigned_tx_instruction_problem1 =
            parse_solana_transaction(&unsigned_tx_instruction_problem1_hex_string, true, None)
                .unwrap();
    ...
    fn solana_fuzzer_regression_test4() {
        ...
        let unsigned_tx_instruction_problem2_string = String::from_utf8(vec![
            ,
